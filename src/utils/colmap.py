# From https://raw.githubusercontent.com/intel-isl/FreeViewSynthesis/33a31ee214a77a2fa074d3a10cedc09803ec2ceb/co/colmap.py
...
class Colmap(object):
    def __init__(self, colmap_path, working_dir, im_paths, colmap_envs=None, camera_model='RADIAL', mapper_args=[]):
        self.colmap_path = colmap_path
        self.colmap_envs = colmap_envs
        ...
        self.mapper_args = [str(x) for x in mapper_args]
    ...
    def _exec_cmd(self, cmd):
        ...
        subprocess.run(cmd, env=self.colmap_envs)
    ...
    def feature_matcher(self):
        cmd = [
            ...
            "1",
            '--ExhaustiveMatching.block_size', '10',    # From david's code
            '--SiftMatching.num_threads', '35',         # From david's code
        ]
        self._exec_cmd(cmd)
    ...
    def mapper(self):
        ...
        cmd = [
            ...
            "--output_path",
            str(self.sparse_dir),
            '--Mapper.max_num_models', '1',             # From david's code
            *self.mapper_args
        ]
        self._exec_cmd(cmd)
    ...
    def write_intrinsics(self, Ks, imdims):
    ...
    def write_calib(self, Ks, Ts, imdims):
        self.write_intrinsics(Ks, imdims)
        self.write_extrinsics(Ts)
...
if __name__ == "__main__":
    ...
    # asin = 'Hasbro_Monopoly_Hotels_Game'
    asin = 'Schleich_Bald_Eagle'

    imgs_dir = f'/private/home/shubhamgoel/.ignition/fuel/fuel.ignitionrobotics.org/GoogleResearch/directdl/{asin}/1/renders_env/'
    im_paths = [f'{imgs_dir}/r_{i}.png' for i in range(50)]

    imgs_dir_out = f'/private/home/shubhamgoel/code/3Dify/colmap_test/{asin}/imgdir/'
    im_paths_out = [f'{imgs_dir_out}/r_{i}.png' for i in range(40)]

    working_dir = f'/private/home/shubhamgoel/code/3Dify/colmap_test/{asin}/working_dir/'
    from pathlib import Path
    Path(imgs_dir_out).mkdir(parents=True, exist_ok=True)
    Path(working_dir).mkdir(parents=True, exist_ok=True)

    import os

    import imageio
    for f, f_out in zip(im_paths, im_paths_out):
        if not os.path.isfile(f_out):
            img = imageio.imread(f)
            imageio.imwrite(f_out, img)

    colmap_path = '/private/home/dnovotny/lib/colmap/usr/local/bin/colmap'
    colmap_envs = {'LD_LIBRARY_PATH': (
                '/private/home/dnovotny/.conda/envs/object_dataset/lib/' # ideally this should get parsed-out automatically from env variables
                ':/usr/lib/x86_64-linux-gnu'
                # ':/public/apps/cuda/10.0/lib64'
                ':/public/apps/cuda/9.2/lib64'
                ':/private/home/dnovotny/lib/colmap_libs'
            )
        }

    colmap = Colmap(colmap_path, working_dir, im_paths_out, colmap_envs=colmap_envs)
